...
namespace ClassicUO.UnitTests.Utility.FileSystemHelper
{
    public class CreateFolderIfNotExists
    {
        ...
        {
            string tempPath = Path.Combine(Path.GetTempPath(), nameof(CreateFolderIfNotExists));

            string createdPath = ClassicUO.Utility.FileSystemHelper.CreateFolderIfNotExists(tempPath, "Part1", "Part2");
