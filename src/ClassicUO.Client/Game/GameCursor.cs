...
namespace ClassicUO.Game
{
    public sealed class GameCursor
    {
        ...
        public GameCursor(World world)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                for (int j = 0; j < 16; j++)
                {
                    ...
                    {
                        // 如果需要缩放，创建放大的surface（ScaleSurface会自动销毁原始surface）
                        if (cursorScale > 1.0f)
                        {
                            surface = ScaleSurface(surface, cursorScale, ref hotX, ref hotY, destroyOriginal: true);
                        }
        ...
        private unsafe nint ScaleSurface(nint originalSurface, float scale, ref int hotX, ref int hotY, bool destroyOriginal = false)
        {
            ...
            if (destroyOriginal)
            {
                SDL.SDL_DestroySurface(originalSurface);
            }
